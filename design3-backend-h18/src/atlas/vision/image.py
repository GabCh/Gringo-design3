...
from atlas.vision.util import Mask, PixelCoordinate
...
class Image:
    ...
    def find_contours(self) -> tuple:
        return cv2.findContours(self.frame, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    def draw_contours(self, contours: list, colour: tuple, thickness: int) -> "Image":
        image_contours = cv2.drawContours(self.frame, contours, -1, colour, thickness)
