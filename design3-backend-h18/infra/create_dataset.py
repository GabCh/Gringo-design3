...
def load_camera_settings():
    subprocess.call(["uvcdynctrl", "-L", "cameraMondeSettings.txt", "-d", args.camera])


def check_dataset_existence_or_create_dir():
    if os.path.isdir(args.base_dir + "/" + args.dataset_name):
        res = messagebox.askokcancel("Warning","Dataset '{}' already exists. Images will be overwritten. Do you want to continue?".format(args.dataset_name))
        if not res:
            quit(0)
    else:
